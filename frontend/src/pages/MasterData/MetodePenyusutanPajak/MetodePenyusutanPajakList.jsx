...
import MetodePenyusutanPajakForm from "./MetodePenyusutanPajakForm";

const MetodePenyusutanPajakList = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get("/api/metode-penyusutan-pajak");
      setData(res.data);
    } catch (err) {
  ...
  const confirmDelete = async () => {
    try {
      await api.delete(`/api/metode-penyusutan-pajak/${confirmDeleteId}`);
      fetchData();
    ...
    <Layout>
      <div className="p-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold">Master Metode Penyusutan Pajak</h2>
          <button
            onClick={handleAdd}
...
export default MetodePenyusutanPajakList;
